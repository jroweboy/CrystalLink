...
class Character extends FlxExtendedSprite {
	...
	private var initialMaxVelocity: FlxPoint;
	private var diaganolMaxVelocity: FlxPoint;
	...
	public function new(Name:String, X:Float = 0, Y:Float = 0, ?JsonPath:String, ?SimpleGraphic:Dynamic) {
		...
		// make the acceleration almost instant
		drag.x = maxVelocity.x * 10;
		drag.y = maxVelocity.y * 10;
		initialMaxVelocity = new FlxPoint(maxVelocity.x, maxVelocity.y);
		diaganolMaxVelocity = new FlxPoint(
					maxVelocity.x / Math.sqrt(2),
					maxVelocity.y / Math.sqrt(2)
			);
	}
